    ...
    <div class="p-3 mb-6 bg-white rounded-lg shadow-sm dark:bg-gray-700 sm:p-4">
        <div class="flex flex-col gap-4">
            <div class="relative w-full">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search authors..."
    ...
    <div wire:loading.remove class="grid w-full gap-3 sm:gap-4 md:gap-5 lg:gap-6"
        ...
        @forelse ($authors as $author)
            <div class="transition-all duration-200 transform hover:scale-102 focus-within:scale-102">
                <x-author-card :name="$author->name" :email="$author->user->email" :books="$author->books()->count()" :photo="$author->user->profile_photo_url
                    ? asset('storage/' . $author->user->profile_photo_url)
