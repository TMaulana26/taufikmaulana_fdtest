    ...
    <div class="p-3 mb-6 bg-white rounded-lg shadow-sm dark:bg-gray-700 sm:p-4">
        <div class="flex flex-col gap-4">
            <div class="relative w-full">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search readers..."
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <select wire:model.live="verificationStatus"
                    ...
                    <option value="verified" class="flex items-center gap-2">
                        {{ __('Verified') }}
                    </option>
                    <option value="unverified" class="flex items-center gap-2">
                        {{ __('Unverified') }}
                    </option>
                </select>
    ...
    <div wire:loading.remove class="grid w-full gap-3 sm:gap-4 md:gap-5 lg:gap-6"
        ...
        @forelse ($readers as $reader)
            <div class="transition-all duration-200 transform hover:scale-102 focus-within:scale-102">
                <x-reader-card :name="$reader->name" :email="$reader->email" :status="$reader->email_verified_at ? 'verified' : 'unverified'" :photo="$reader->profile_photo_url
                    ? asset('storage/' . $reader->profile_photo_url)
